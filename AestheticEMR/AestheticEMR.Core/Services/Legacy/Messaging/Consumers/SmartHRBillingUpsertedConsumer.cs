...
/// <summary>
/// Idempotently applies BillingUpsertedEvent to the SmartHR database.
/// Safe to replay: uses MERGE on BillNo (PK).
/// </summary>
public class SmartHRBillingUpsertedConsumer(
    IConfiguration configuration,
    ILogger<SmartHRBillingUpsertedConsumer> logger) : IConsumer<BillingUpsertedEvent>
{
    public async Task Consume(ConsumeContext<BillingUpsertedEvent> context)
    {
        ...
        {
            logger.LogWarning("SmartHRConnection not configured; skipping SmartHR billing sync for {BillNo}.", context.Message.BillNo);
            return;
        }
        ...
        logger.LogInformation("SmartHR billing sync completed for {BillNo}.", context.Message.BillNo);
    }
