          ...
          <div class="tab-body">
            ...
            <div class="span-2 image-toolbar">
              ...
              @if (selectedImageFiles.length) {
                <span class="image-count">{{ selectedImageFiles.length }} image(s) selected</span>
              }
            </div>
...
export class DentalEncounterDialogComponent {
  ...
  get selectedAttendanceSummary(): VwhRecord | undefined {
    const patient = this.selectedPatientInfo;
    if (!patient) return undefined;

    return {
      consultId: patient.consultId,
      pNo: patient.pNo,
      clientCat: patient.clientCat,
      clinicType: patient.clinic || '',
      fullname: patient.fullName || '—',
      dob: patient.dateOfBirth,
      retainName: patient.companyName,
      coyname: patient.coyId || patient.companyName,
      patientPhotoBase64: patient.photo
    };
  }
  ...
  private applyLegacyFlagsToStatusMap(): void {
    ...
    const dtype = this.normalizeDtype(this.chart.dtype);

    const dtypeKey: keyof ToothStatus | null =
      dtype === 'present' ? 'present'
        : dtype === 'carious' ? 'carious'
          : dtype === 'decayed' ? 'decayed'
  ...
  private normalizeDtype(dtype?: string): 'present' | 'carious' | 'decayed' | 'missing' | 'filled' | '' {
    switch ((dtype || '').trim().toUpperCase()) {
      case 'P':
      case 'TEETH PRESENT':
        return 'present';
      case 'C':
