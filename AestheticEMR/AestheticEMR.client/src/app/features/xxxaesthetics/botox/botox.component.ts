...
export class BotoxComponent {
  ...
  private async saveConsultation(result: BotoxDialogResult): Promise<void> {
    ...
    try {
      ...
      if (!consultation.id) {
        const consultId = consultation.consultId ?? '';
        const hasConsent = await this.checkSignedConsent(consultId, consultation.pNo ?? '', 'Botox');
        if (!hasConsent) {
          this.alertService.stopLoadingMessage();
          this.loadingIndicator = false;
          this.alertService.showStickyMessage(
            'Consent Required',
            'A signed consent form is required before saving a Botox session. Please complete the patient consent form first.',
