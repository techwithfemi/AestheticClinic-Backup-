...
@Component({
  selector: 'app-aesthetics-consultations-report',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './aesthetics-consultations-report.component.html',
  styleUrl: './aesthetics-consultations-report.component.scss'
})
export class AestheticsConsultationsReportComponent implements OnInit {
  ...
  readonly filteredRows = computed(() => {
    ...
    return this.dateFiltered().filter(c =>
      ...
      || this.resolveUserName(c.provider).toLowerCase().includes(term)
    );
  });
  ...
  readonly consultationMeta = computed(() => {
    const c = this.selectedConsultation();
    if (!c) {
      return {
        patientName: '—', pNo: '—', consultId: '—', procedure: '—',
        date: '—', user: '—', service: '—', status: 'Pending'
      };
    }

    return {
      patientName: this.resolvePatientName(c),
      pNo: c.pNo || '—',
      consultId: c.consultId || '—',
      procedure: c.procedureType || 'Not documented',
      date: c.consultationDate || 'Not documented',
      user: this.resolveUserName(c.provider),
      service: c.services || 'Not documented',
      status: c.consentGiven ? 'Completed' : 'Pending'
    };
  });
  ...
  async load(): Promise<void> {
    ...
    this.alertService.startLoadingMessage('Loading consultations report...');
    ...
    try {
      ...
      this.loadingIndicator = false;
      this.alertService.stopLoadingMessage();
      this.alertService.showStickyMessage('Load Error', `Unable to load consultations report.\r\nError: "${error?.message ?? error}"`, MessageSeverity.error, error);
    }
  }
  ...
  displayReport(): void {
    ...
    const consultation = this.consultations().find(c => (c.consultId ?? '').trim().toLowerCase() === selected.consultId.toLowerCase() && (c.pNo ?? '').trim().toLowerCase() === selected.pNo.toLowerCase())
      || this.consultations()
  ...
  exportExcel(event: Event): void {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Consultations Report');
    ...
    this.downloadBlob(blob, this.buildFileName('aesthetics-consultations-report', 'xlsx'));
  }
  ...
  exportCsv(event: Event): void {
    ...
    this.downloadBlob(blob, this.buildFileName('aesthetics-consultations-report', 'csv'));
  }
  ...
  exportPdf(event: Event): void {
    ...
    const lines = [
      'Aesthetics Consultations Report',
    ...
    this.downloadBlob(blob, this.buildFileName('aesthetics-consultations-report', 'pdf'));
  }
  ...
  private getExportRows(): Array<Record<string, string>> {
    return this.filteredRows().map(row => ({
      ...
      User: this.resolveUserName(row.provider),
      Status: row.consentGiven ? 'Completed' : 'Pending'
    }));
  }
