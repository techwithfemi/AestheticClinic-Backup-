...
import { AestheticSignedConsent } from '../../../models/aesthetic.model';
...
@Component({
  selector: 'app-aesthetics-consent-report',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './aesthetics-consent-report.component.html',
  styleUrl: './aesthetics-consent-report.component.scss'
})
export class AestheticsConsentReportComponent implements OnInit {
  ...
  readonly displayedColumns = ['date', 'patient', 'consultId', 'procedure', 'user', 'status'];

  generatedOn = new Date();
  readonly selectedConsent = signal<AestheticSignedConsent | null>(null);

  readonly consent = computed(() => {
    const c = this.selectedConsent();
    if (!c) {
      return {
        patientName: '—', pno: '—', consultId: '—', procedureDetail: 'Not documented',
        templateName: 'Not documented', consented: false, signedAt: 'Not signed',
        providerName: 'Not documented', witnessName: 'Not documented', date: 'Not documented'
      };
    }

    const patientName = this.resolvePatientNameByPNo(c.pNo || '') || '—';
    return {
      patientName,
      pno: c.pNo || '—',
      consultId: c.consultId || '—',
      procedureDetail: c.procedureType || 'Not documented',
      templateName: c.consentTemplateId ? `Template #${c.consentTemplateId}` : 'Not documented',
      consented: !c.isVoided,
      signedAt: c.signedDate || 'Not signed',
      providerName: this.resolveUserName(c.signedBy),
      witnessName: c.witnessedBy || 'Not documented',
      date: c.signedDate ? c.signedDate.split('T')[0] : 'Not documented'
    };
  });

  readonly sections = computed<ConsentSection[]>(() => {
    const c = this.selectedConsent();
    const content = c?.consentContent || '';
    return [
      {
        title: 'Procedure Description',
        content: this.extractSection(content, 'Procedure Description') || content || 'Not documented.'
      },
      {
        title: 'Risks and Complications',
        content: this.extractSection(content, 'Risks and Complications') || c?.notes || 'Not documented.'
      },
      {
        title: 'Post-Treatment Care Instructions',
        content: this.extractSection(content, 'Post-Treatment Care Instructions') || 'Not documented.'
      }
    ];
  });
  ...
  readonly dateFiltered = computed(() => {
    ...
    }).sort((a, b) => new Date(b.signedDate ?? 0).getTime() - new Date(a.signedDate ?? 0).getTime());
  });
  ...
  readonly filteredRows = computed(() => {
    ...
    return this.dateFiltered().filter(c =>
      this.resolvePatientNameByPNo(c.pNo || '').toLowerCase().includes(term)
      || (c.consultId ?? '').toLowerCase().includes(term)
      || (c.procedureType ?? '').toLowerCase().includes(term)
      || this.resolveUserName(c.signedBy).toLowerCase().includes(term)
    );
  });
  ...
  async load(): Promise<void> {
    ...
    this.alertService.startLoadingMessage('Loading consent report...');
    ...
    try {
      ...
      const [attendances, patients, consents, users] = await Promise.all([
        ...
        firstValueFrom(this.aestheticEndpoint.getSignedConsentsEndpoint<AestheticSignedConsent[]>({ includeVoided: true })),
        usersPromise
      ]);
      ...
      this.loadingIndicator = false;
      this.alertService.stopLoadingMessage();
      this.alertService.showStickyMessage('Load Error', `Unable to load consent report.\r\nError: "${error?.message ?? error}"`, MessageSeverity.error, error);
    }
  }
  ...
  displayReport(): void {
    ...
    const consent = this.consents()
      .filter(c => (c.consultId ?? '').trim().toLowerCase() === selected.consultId.toLowerCase() && (c.pNo ?? '').trim().toLowerCase() === selected.pNo.toLowerCase())
      .sort((a, b) => new Date(b.signedDate ?? 0).getTime() - new Date(a.signedDate ?? 0).getTime())[0];

    if (!consent) {
      this.alertService.showStickyMessage('No Data', `No signed consent found for ${selected.patientName} [${selected.consultId}].`, MessageSeverity.warn);
      this.selectedConsent.set(null);
      return;
    }
  ...
  exportExcel(event: Event): void {
    ...
    this.downloadBlob(blob, this.buildFileName('aesthetics-consent-report', 'xlsx'));
  }
  ...
  exportCsv(event: Event): void {
    ...
    this.downloadBlob(blob, this.buildFileName('aesthetics-consent-report', 'csv'));
  }
  ...
  exportPdf(event: Event): void {
    ...
    this.downloadBlob(blob, this.buildFileName('aesthetics-consent-report', 'pdf'));
  }

  resolvePatientNameByPNo(pNo: string): string {
  ...
  resolveUserName(userIdOrName: string | undefined): string {
    if (!userIdOrName?.trim()) return '—';
    const value = userIdOrName.trim();
  ...
  private getExportRows(): Array<Record<string, string>> {
    return this.filteredRows().map(row => ({
      Date: this.formatDate(row.signedDate),
      Patient: this.resolvePatientNameByPNo(row.pNo || ''),
      'Consult ID': row.consultId || '—',
      Procedure: row.procedureType || '—',
      User: this.resolveUserName(row.signedBy),
