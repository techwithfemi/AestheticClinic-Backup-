...
interface SkinAssessmentFinding {
  label: string;
  present: boolean;
}
...
@Component({
  selector: 'app-aesthetics-skin-assessment-report',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './aesthetics-skin-assessment-report.component.html',
  styleUrl: './aesthetics-skin-assessment-report.component.scss'
})
export class AestheticsSkinAssessmentReportComponent implements OnInit {
  ...
  readonly displayedColumns = ['date', 'patient', 'consultId', 'procedure', 'provider', 'notes'];
  ...
  readonly attendanceOptions = computed(() => {
    ...
    return this.attendances()
      ...
      .map(a => {
        ...
        const patientName = this.resolvePatientNameByPNo(pNo);
        return {
          key: `${consultId}|${pNo}`,
          consultId,
          pNo,
  ...
  readonly patient = computed(() => {
    ...
    if (!selected) {
      return { name: '—', pno: '—', dob: '—', consultationDate: '—', provider: '—' };
    }

    const aPatient = this.aestheticPatients().find(p => p.id === selected.patientId);
    return {
      name: this.resolvePatientName(selected),
      pno: selected.pNo || aPatient?.pno || '—',
      dob: aPatient?.dateOfBirth || 'Not documented',
      consultationDate: selected.consultationDate || 'Not documented',
      provider: this.resolveProviderName(selected.provider)
    };
  });

  readonly skinAssessment = computed<SkinAssessmentFinding[]>(() => this.parseSkinAssessment(this.selectedConsultation()?.skinAssessment));

  readonly skinDetails = computed(() => {
    const source = this.selectedConsultation()?.skinAssessment;
    return {
      fitzpatrickSkinType: this.extractValue(source, 'fitzpatrick') || 'Not documented',
      acneSeverity: this.extractValue(source, 'acne severity') || 'Not documented',
      pigmentation: this.extractValue(source, 'pigmentation') || 'Not documented',
      scarring: this.extractValue(source, 'scarring') || 'Not documented',
      volumeLoss: this.extractValue(source, 'volume loss') || 'Not documented'
    };
  });

  readonly medicalHistory = computed<MedicalHistoryItem[]>(() => {
    const selected = this.selectedConsultation();
    if (!selected) {
      return [
        { label: 'Allergies', value: 'Not documented' },
        { label: 'Current Medications', value: 'Not documented' },
        { label: 'Past/Medical History', value: 'Not documented' },
        { label: 'HSV History', value: 'Not documented' },
        { label: 'Pregnancy', value: 'Not documented' }
      ];
    }

    const patient = this.aestheticPatients().find(p => p.id === selected.patientId);
    return [
      { label: 'Allergies', value: selected.allergies || patient?.allergies || 'Not documented' },
      { label: 'Current Medications', value: selected.currentMedications || patient?.currentMedications || 'Not documented' },
      { label: 'Past/Medical History', value: patient?.medicalHistory || 'Not documented' },
      { label: 'HSV History', value: this.extractBooleanHint(selected.skinAssessment, 'hsv history') },
      { label: 'Pregnancy', value: this.extractBooleanHint(selected.skinAssessment, 'pregnancy') }
    ];
  });

  readonly treatmentPlan = computed<TreatmentPlanItem[]>(() => {
    const selected = this.selectedConsultation();
    if (!selected) {
      return [{ area: 'Not documented', modality: 'Not documented', objective: 'Not documented' }];
    }

    return [{
      area: selected.areaTreated || 'Face',
      modality: selected.procedureType || 'Aesthetic Procedure',
      objective: selected.treatmentPlan || selected.procedureDescription || 'Not documented'
    }];
  });

  readonly notes = computed(() => this.selectedConsultation()?.postTreatmentInstructions || this.selectedConsultation()?.followUpReview || 'No additional note.');
  ...
  async load(): Promise<void> {
    ...
    this.alertService.startLoadingMessage('Loading skin assessment report...');
    ...
    try {
      ...
      this.loadingIndicator = false;
      this.alertService.stopLoadingMessage();
      this.alertService.showStickyMessage('Load Error', `Unable to load skin assessment report.\r\nError: "${error?.message ?? error}"`, MessageSeverity.error, error);
    }
  }
  ...
  displayReport(): void {
    ...
    const consultation = this.consultations().find(c => (c.consultId ?? '').trim().toLowerCase() === selected.consultId.toLowerCase())
      || this.consultations()
    ...
    if (!consultation) {
      this.alertService.showStickyMessage('No Data', `No consultation found for ${selected.patientName} [${selected.consultId}].`, MessageSeverity.warn);
      this.selectedConsultation.set(null);
      return;
    }
  ...
  exportExcel(event: Event): void {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Skin Assessment Report');
    ...
    this.downloadBlob(blob, this.buildFileName('aesthetics-skin-assessment-report', 'xlsx'));
  }
  ...
  exportCsv(event: Event): void {
    ...
    this.downloadBlob(blob, this.buildFileName('aesthetics-skin-assessment-report', 'csv'));
  }
  ...
  exportPdf(event: Event): void {
    ...
    const lines = [
      'Aesthetics Skin Assessment Report',
      `Generated: ${this.formatDate(new Date())}`,
      `Records: ${rows.length}`,
      '',
      'Date | Patient | Consult ID | Procedure | Provider'
    ];

    for (const row of rows) {
      const line = `${row['Date']} | ${row['Patient']} | ${row['Consult ID']} | ${row['Procedure']} | ${row['Provider']}`;
      lines.push(line.length > 145 ? `${line.slice(0, 142)}...` : line);
    }

    const blob = new Blob([this.buildSimplePdf(lines)], { type: 'application/pdf' });
    this.downloadBlob(blob, this.buildFileName('aesthetics-skin-assessment-report', 'pdf'));
  }
  ...
  private getExportRows(): Array<Record<string, string>> {
    return this.filteredRows().map(row => ({
      ...
      Provider: this.resolveProviderName(row.provider),
  ...
  private getDefaultSkinAssessment(): SkinAssessmentFinding[] {
    return [
      { label: 'Acne', present: false },
      { label: 'Rosacea', present: false },
      { label: 'Hyperpigmentation', present: false },
      { label: 'Scarring', present: false },
      { label: 'Volume loss', present: false },
      { label: 'Wrinkles/Fine lines', present: false },
      { label: 'Skin laxity', present: false },
      { label: 'Keloid tendency', present: false }
    ];
  }

  private parseSkinAssessment(skinAssessment?: string): SkinAssessmentFinding[] {
    const base = this.getDefaultSkinAssessment();
    if (!skinAssessment) return base;

    const normalized = skinAssessment.toLowerCase();
    const checks: Record<string, string[]> = {
      Acne: ['acne'],
      Rosacea: ['rosacea'],
      Hyperpigmentation: ['hyperpigmentation', 'pigmentation'],
      Scarring: ['scar', 'scarring'],
      'Volume loss': ['volume loss'],
      'Wrinkles/Fine lines': ['wrinkle', 'fine line'],
      'Skin laxity': ['laxity'],
      'Keloid tendency': ['keloid']
    };

    return base.map(item => ({ ...item, present: (checks[item.label] || []).some(k => normalized.includes(k)) }));
  }

  private extractValue(source: string | undefined, key: string): string | null {
  ...
  private extractBooleanHint(source: string | undefined, key: string): string {
    return this.extractValue(source, key) || 'Not documented';
  }
}
