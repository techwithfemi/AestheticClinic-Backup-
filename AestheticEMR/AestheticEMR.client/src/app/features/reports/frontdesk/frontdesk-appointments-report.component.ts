...
@Component({
  selector: 'app-frontdesk-appointments-report',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './frontdesk-appointments-report.component.html',
  styleUrl: './frontdesk-appointments-report.component.scss'
})
export class FrontdeskAppointmentsReportComponent implements OnInit {
  ...
  private readonly appliedClinic   = signal<string>('');
  readonly appliedSearch           = signal<string>('');

  readonly pageSize    = 10;
  readonly currentPage = signal<number>(0);

  readonly displayedColumns = ['date', 'time', 'patient', 'clinicType', 'employee', 'remarks'];

  // ---- Computed: appointments matching APPLIED date/patient/clinic filters ----
  readonly dateFiltered = computed(() => {
    ...
    const clinic = this.appliedClinic();
    if (clinic) data = data.filter(a => (a.clinicType ?? '') === clinic);
    ...
    data = data.filter(a => {
      ...
      const t = new Date(a.apptDate).getTime();
      return t >= from.getTime() && t <= to.getTime();
    });

    return data.sort((a, b) =>
      new Date(b.apptDate ?? 0).getTime() - new Date(a.apptDate ?? 0).getTime()
    );
  });
  ...
  readonly totalAppointments = computed(() => this.filtered().length);
  readonly todayAppointments = computed(() => this.filtered().filter(a => this.isToday(a.apptDate)).length);
  ...
  // ---- Clinic types for dropdown (from loaded appointments) ----
  readonly clinicTypes = computed(() => {
    const types = this.appointments().map(a => a.clinicType ?? '').filter(s => s.trim());
  ...
  load(): void {
    ...
    this.alertService.startLoadingMessage('Loading appointments report...');
    ...
    Promise.all([
      this.appointmentEndpoint.getAppointmentsEndpoint<Appointment[]>().toPromise(),
      patientsPromise,
      visitsPromise
    ]).then(([appointments, patients, visits]) => {
      ...
      this.alertService.showStickyMessage(
        'Load Error',
        `Unable to load appointments report.\r\nError: "${error?.message ?? error}"`,
  ...
  runReport(): void {
    ...
    this.appliedClinic.set(this.selectedClinic);
  ...
  exportExcel(event: Event): void {
    ...
    this.downloadBlob(blob, this.buildFileName('frontdesk-appointments-report', 'xlsx'));
  }
  ...
  exportCsv(event: Event): void {
    ...
    const headers = ['Date', 'Time', 'Patient No', 'Patient', 'Clinic', 'Employee', 'Remarks'];
    const csvLines = [
      headers.join(','),
      ...rows.map(r => [r.date, r.time, r.pno, r.patient, r.clinic, r.employee, r.remarks]
        .map(v => this.escapeCsv(v)).join(','))
    ];
    const csvContent = '﻿' + csvLines.join('\r\n');
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    this.downloadBlob(blob, this.buildFileName('frontdesk-appointments-report', 'csv'));
  }
  ...
  exportPdf(event: Event): void {
    ...
    const lines = [
      'Frontdesk Appointments Report',
      `Generated: ${this.formatDate(new Date())}`,
      `Records: ${rows.length}`,
      '',
      'Date | Time | Patient | Clinic | Employee'
    ];
    for (const r of rows) {
      const line = `${r.date} | ${r.time} | ${r.patient} | ${r.clinic} | ${r.employee}`;
      lines.push(line.length > 145 ? `${line.slice(0, 142)}...` : line);
    }

    const pdfContent = this.buildSimplePdf(lines);
    const blob = new Blob([pdfContent], { type: 'application/pdf' });
    this.downloadBlob(blob, this.buildFileName('frontdesk-appointments-report', 'pdf'));
  }

  private getExportRows(): Array<{
    date: string; time: string; pno: string; patient: string;
    clinic: string; employee: string; remarks: string;
  }> {
    return this.filtered().map(a => ({
      date: this.formatDate(a.apptDate),
      time: this.formatTime(a.apptTime),
      pno: a.pno ?? '',
