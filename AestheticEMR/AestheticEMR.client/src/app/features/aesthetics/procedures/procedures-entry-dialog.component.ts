  ...
  template: `
    <div class="procedures-page">
      ...
      <app-dialog-message-banner
        ...
        [autoHideMs]="5000"
        (visibleChange)="dialogBannerVisible.set($event)">
      </app-dialog-message-banner>
          ...
          @case ('duplicate') {
            <div class="alert alert-warning duplicate-alert">
              ...
              <div class="alert-content">
                ...
                <p>{{ alert.message }}</p>
                @if (alert.actionLabel && alert.action) {
                  <button mat-stroked-button (click)="acknowledgeDuplicateAlert(alert)" class="alert-action">{{ alert.actionLabel }}</button>
                }
              </div>
            </div>
          }
              ...
              <div class="tab-body consent-tab">
                ...
                <div class="full consent-preview">
                  ...
                  @if (!hasConsentSelection()) {
                    <div class="empty-consent">Select a procedure type to view signed consent.</div>
            ...
            <mat-tab>
              ...
              <div class="tab-body" [formGroup]="consultationGroup">
                ...
                <mat-form-field appearance="outline" class="half">
                  ...
                  <mat-select formControlName="medications" multiple (selectionChange)="onMedicationsChanged()">
                  ...
                  @if (saveAttempted() && applySaveBlockers() && !hasCurrentMedications()) {
                    <mat-error>Current medications are required for safety.</mat-error>
                  }
                </mat-form-field>
                ...
                <mat-form-field appearance="outline" class="half">
                  ...
                  @if (saveAttempted() && applySaveBlockers() && !hasAllergyInformation()) {
                    <mat-error>Allergy information is required for safety.</mat-error>
                  }
                </mat-form-field>
...
export class ProceduresEntryDialogComponent implements OnInit {
  ...
  readonly selectedAttendanceSummary = signal<VwhRecord | null>(null);
  ...
  readonly patientAttendanceOptions = computed<{
    ...
    const options = this.todayVisits()
    ...
    if (consultId && pNo && !options.some(item => item.consultId === consultId && item.pNo === pNo)) {
      const patient = this.findPatientByAttendancePno(patients, pNo);
      const patientName = (consultation?.patientName || '').trim()
        || this.resolveAttendancePatientName({ pNo, fullname: consultation?.patientName || '' }, patient);

      options.unshift({
        trackKey: `consultation-${consultId}-${pNo}`,
        consultId,
        patientId: consultation?.patientId ?? patient?.id ?? 0,
        pNo,
  ...
  readonly applySaveBlockers = signal(true);
  
  readonly dialogBannerVisible = signal(false);
  readonly dialogBannerTitle = signal('');
  readonly dialogBannerMessage = signal('');
  readonly dialogBannerType = signal<DialogMessageBannerType>('info');
  ...
  ngOnInit(): void {
    ...
    // Attendance summary source of truth is attendance endpoint by ConsultId.
    if (this.data.consultation) {
      ...
      this.selectedVisitConsultId.set((this.data.selectedVisitConsultId || '').trim());
    }

    const initialConsultId = this.selectedVisitConsultId().trim();
    if (initialConsultId) {
      this.fetchAttendanceSummary(initialConsultId);
    } else {
  ...
  private loadAttendances(): void {
    this.attendanceEndpoint.getTodayVisitsEndpoint<QryhvisitsForToday[]>().subscribe({
      next: visits => {
        ...
        if (!this.data.consultation && this.selectedVisitConsultId().trim()) {
  ...
  private loadProcedureTypes(): void {
    this.moduleSettings.getModuleSettings<{ autoFollowUpDays: number; procedureTypes?: string[]; applySaveBlockers?: boolean }>('aesthetics', {
      autoFollowUpDays: 14,
      procedureTypes: ['Procedures', 'Neuromodulator', 'Dermal Filler', 'Laser'],
      applySaveBlockers: true
    }).then(settings => {
      const defaults = ['Procedures', 'Neuromodulator', 'Dermal Filler', 'Laser'];
      const list = (settings.procedureTypes || []).map(x => (x || '').trim()).filter(Boolean);
      const currentProcedureType = (this.data.consultation?.procedureType || '').trim();
      const merged = Array.from(new Set([...(list.length > 0 ? list : defaults), ...(currentProcedureType ? [currentProcedureType] : [])]));
      this.procedureTypes.set(merged.length > 0 ? merged : defaults);
      this.applySaveBlockers.set(settings.applySaveBlockers !== false);
    });
  }

  private fetchAttendanceSummary(consultId: string): void {
    ...
    this.billingEndpoint.getVwhRecordSummaryEndpoint<VwhRecord>(normalizedConsultId).subscribe({
      next: (vwhRecord) => {
        if (!vwhRecord) {
          this.alertService.showStickyMessage(
            'Attendance Not Found',
            `No attendance record found for ConsultId: ${normalizedConsultId}. Please ensure patient attendance has been recorded first.`,
        ...
        this.selectedAttendanceSummary.set(vwhRecord);
      },
      error: (error) => {
        this.alertService.showStickyMessage(
          'Attendance Load Error',
          `Unable to load attendance details for ConsultId: ${normalizedConsultId}. Please check the patient attendance record.`,
  ...
  onMedicationsChanged(): void {
    const rawSelected = this.consultationGroup.get('medications')?.value || [];
    const selected = (Array.isArray(rawSelected) ? rawSelected : [rawSelected])
      .map(x => `${x ?? ''}`.trim().toLowerCase())
      .filter(Boolean);

    const normalized = selected.includes('none') ? ['none'] : selected;
    this.consultationGroup.get('medications')?.setValue(normalized, { emitEvent: false });
  }

  onAllergiesChanged(): void {
    const rawSelected = this.consultationGroup.get('allergySelections')?.value || [];
    const selected = (Array.isArray(rawSelected) ? rawSelected : [rawSelected])
      .map(x => `${x ?? ''}`.trim().toLowerCase())
      .filter(Boolean);
    ...
    this.consultationGroup.get('allergySelections')?.setValue(normalized, { emitEvent: false });
    this.patientAllergies.set(normalized);
    this.validateAllergiesAndDuplicates();
  }
...
- This creates a safety incident record for review`;
  ...
  onPatientVisitChanged(consultId: string): void {
    ...
    this.patientSelectionError.set('');
    ...
    // Fetch attendance summary for the selected visit
    if (normalizedConsultId) {
      this.fetchAttendanceSummary(normalizedConsultId);
    } else {
      this.selectedAttendanceSummary.set(null);
    }

    if (normalizedConsultId && !this.selectedVisitPNo().trim()) {
      this.patientSelectionError.set('PNo is required for selected patient.');
      this.signedConsents.set([]);
      this.alertService.showStickyMessage('Validation error', 'PNo is required for selected patient.', MessageSeverity.warn);
      return;
    }
  ...
  dismissSafetyAlert(alert: SafetyAlert): void {
    this.safetyAlerts.update(current => current.filter(a => a !== alert));
  }
  
  acknowledgeDuplicateAlert(alert: SafetyAlert): void {
    alert.action?.();
    this.dismissSafetyAlert(alert);
  }
  ...
  saveOrUpdate(): void {
    ...
    this.dialogBannerVisible.set(false);
    ...
    if (!pNo) {
      this.patientSelectionError.set('PNo is required. Select a patient with valid PNo.');
      this.alertService.showStickyMessage('Validation error', 'PNo is required. Select a patient with valid PNo.', MessageSeverity.warn);
      this.showDialogBanner('Validation error', 'PNo is required. Select a patient with valid PNo.', 'error');
      return;
    }

    if (this.applySaveBlockers()) {
      const safetyErrors: string[] = [];
      if (!this.hasAllergyInformation()) {
        safetyErrors.push('Allergy information is required for safety.');
      }
      if (!this.hasCurrentMedications()) {
        safetyErrors.push('Current medications are required for safety.');
      }

      if (safetyErrors.length > 0) {
        this.selectedTabIndex.set(1);
        this.alertService.showStickyMessage('Validation error', safetyErrors.join(' '), MessageSeverity.warn);
        this.showDialogBanner('Validation error', safetyErrors.join(' '), 'error');
        return;
    }
    }

    if (this.form.controls.services.invalid || !consultId || !providerEmpId || !procedureType || !consultationDate) {
      this.alertService.showStickyMessage('Validation error', 'Please complete required fields.', MessageSeverity.warn);
      this.showDialogBanner('Validation error', 'Please complete required fields.', 'error');
      return;
    }
    ...
    if (this.hasHardStopAllergies()) {
      ...
      this.showDialogBanner('Hard stop', 'This patient has documented allergies that are contraindicated for the selected procedures. Clarify with patient or select alternative procedures.', 'error');
      return;
    }
    ...
    consultationRequest.subscribe({
      ...
      error: error => {
        ...
        if (this.isPatientPNoErrorMessage(apiMessage)) {
          const message = apiMessage || 'PNo is required. Select a patient with valid PNo.';
          this.patientSelectionError.set(message);
          this.alertService.showStickyMessage('Validation error', message, MessageSeverity.warn, error);
          this.showDialogBanner('Validation error', message, 'error');
          return;
        }

        this.alertService.showStickyMessage('Save error', apiMessage || 'Unable to save procedures.', MessageSeverity.error, error);
        this.showDialogBanner('Save error', apiMessage || 'Unable to save procedures.', 'error');
      }
    });
  }
...
`;
  ...
  hasAllergyInformation(): boolean {
    const consultation = this.consultationGroup.getRawValue();
    const selections = Array.isArray(consultation.allergySelections)
      ? consultation.allergySelections.map(x => `${x ?? ''}`.trim().toLowerCase()).filter(Boolean)
      : [];
    const notes = `${consultation.allergyNotes ?? ''}`.trim();
    return selections.includes('none') || selections.length > 0 || notes.length > 0;
  }

  hasCurrentMedications(): boolean {
    const consultation = this.consultationGroup.getRawValue();
    const medications = Array.isArray(consultation.medications)
      ? consultation.medications.map(x => `${x ?? ''}`.trim().toLowerCase()).filter(Boolean)
      : [];
    return medications.includes('none') || medications.length > 0;
  }
  ...
  private loadFromConsultation(consultation?: AestheticConsultation): void {
    ...
    this.form.controls.services.setValue((consultation.services || '').trim());
  ...
  private buildPayload(): object {
    ...
    return {
      ...
      Services: services,
      Allergies: consultation.allergyNotes
        || (consultation.allergySelections
          ? consultation.allergySelections
            .map(x => `${x ?? ''}`.trim())
            .filter(Boolean)
            .join(', ')
          : ''),
      CurrentMedications: consultation.medications
  ...
  private finishSaveSuccess(failedUploads = 0): void {
    ...
    this.showDialogBanner('Success', message, failedUploads > 0 ? 'warning' : 'success');
    this.dialogRef.close(true);
  }

  private showDialogBanner(title: string, message: string, type: DialogMessageBannerType): void {
    this.dialogBannerTitle.set(title);
    this.dialogBannerMessage.set(message);
    this.dialogBannerType.set(type);
    this.dialogBannerVisible.set(true);
  }
  ...
  private isPatientPNoErrorMessage(message: string): boolean {
    const m = (message || '').toLowerCase();
    return m.includes('patient is required')
      || m.includes('patient number (pno) is required')
      || m.includes('pno is required')
      || m.includes('no patient record found for pno')
      || m.includes('no patient record found for the provided pno');
  }
